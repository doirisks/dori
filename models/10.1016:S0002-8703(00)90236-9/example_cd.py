...
contains code to compare models c and d. 
10.1016:S0002-8703(00)90236-9
2000 Primary and subsequent coronary risk appraisal: New results from The Framingham Study

d should estimate c, but it seems inaccurate for some inputs.
"""

from model_d import model as d
from model_c import model as c
...
toscore = [
    ...
    [61,200,72,144,1./3,234,1,0,0,1],    
    [55,180,81,144,4./3,130,0,0,0,1],    
    [55,180,81,120,4./3,130,0,0,0,1],    
    [72,230,67,120,4./3,130,1,0,0,1],    
    [72,230,67,120,4./3,130,1,1,0,1],    
    [61,220,43,118,1./3,234,1,1,0,0],    
]
cscores = []
dscores = []
for i in range(len(toscore)):
    cscores.append(c(toscore[i][0],toscore[i][1],toscore[i][2],toscore[i][3],toscore[i][4],toscore[i][5],toscore[i][6],toscore[i][7],toscore[i][8],toscore[i][9]))
    dscores.append(d(toscore[i][0],toscore[i][1],toscore[i][2],toscore[i][3],toscore[i][4],toscore[i][5],toscore[i][6],toscore[i][7],toscore[i][8],toscore[i][9]))
    # shows the two different scores and their ratio. Notice that the ratio grows large whenever the 120 < sysBP < 200 and antihypertensives are in use.
    print "%.3f" % (float(cscores[i])*100.), "%.3f" % (float(dscores[i])*100.), "%.3f" % (float(cscores[i])/dscores[i])
